...
class TestScheduledTransferForm:
    ...
    def test_flow_queryset_includes_other_users_flows(self, regular_user, admin_user, make_flow):
        make_flow(admin_user, name='AdminFlow')
        form = ScheduledTransferForm(user=regular_user)
        assert form.fields['flow'].queryset.filter(name='AdminFlow').count() == 1
